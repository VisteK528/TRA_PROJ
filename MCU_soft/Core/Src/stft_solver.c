...
void STFT_Normalize(STFT_with_filter_solver *stft_solver) {
    float max_value = 0.0f;

    for (uint32_t i = 0; i < stft_solver->out_length; i++) {
        if (fabsf(stft_solver->out[i]) > max_value) {
            max_value = fabsf(stft_solver->out[i]);
        }
    }

    if (max_value > 0.0f) {
        for (uint32_t i = 0; i < stft_solver->out_length; i++) {
            stft_solver->out[i] /= max_value;
        }
    }
}
